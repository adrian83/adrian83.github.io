...
If you know how what [higher-order function](https://en.wikipedia.org/wiki/Higher-order_function) are you will easily understand how docorators implemented as functions work.
...
Thing that can make working with decorators a bit harder is using multiple decorators on one function / method. Below you can see such case.
...
If you look at the output you will see that first line generated by @log2 decorator prints *Executing 'wrapper' with...*. 
As you can see @log2 is actually decorating @log decorator. 



#### Python decorators implemented as classes

Decorators can be aslo implemented as classes which at first glance may look like something quite different than what we saw above. 
Actually Python contains 'magic method' called `__call__` which allows class instances behave like functions. 
This makes both ways of creating decorators very similar.
...
as in the corresponding example above we can see additional step (#1) which is creation of decorator with two parameters.  
