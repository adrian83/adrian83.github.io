...
If you know what [higher-order functions](https://en.wikipedia.org/wiki/Higher-order_function) and [closures](https://en.wikipedia.org/wiki/Closure_(computer_programming)) are, you will quickly understand, how decorators work. Let’s see simple example.


##### 1. Decorator without its own parameters
```
def log(func):
...
##### 2. Decorator with its own parameters
```
...
Again, as in the corresponding example above (decorators implemented as functions) we see, that @Log2 decorator is decorating @Log.
